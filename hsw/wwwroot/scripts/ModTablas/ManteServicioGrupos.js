...
var tabla = $('#tabla').DataTable({
    ...
    columnDefs: [
        {
            ...
            render: function (data, type, full, meta) {
                return (
                    ...
                    '<a data-bs-toggle="tooltip" class="btn btn-sm btn-icon btn-text-secondary waves-effect waves-light rounded-pill" data-bs-placement="top" title="Consultar grupo de servicio"><i class="ri-eye-line ri-20px"          style="color:blue"  data-opc="2"></i></a>' +
                    '<a data-bs-toggle="tooltip" class="btn btn-sm btn-icon btn-text-secondary waves-effect waves-light rounded-pill" data-bs-placement="top" title="Editar grupo de servicio"   ><i class="ri-pencil-line ri-20px"       style="color:green" data-opc="3"></i></a>' +
                    '<a data-bs-toggle="tooltip" class="btn btn-sm btn-icon btn-text-secondary waves-effect waves-light rounded-pill" data-bs-placement="top" title="Anular grupo de servicio"   ><i class="ri-delete-bin-7-line ri-20px" style="color:red"   data-opc="4"></i></a>' +
                    '</div>'
                );
            }
        },
        ...
        { "width": "210px", "targets": [5, 7], className: "text-center" }
    ],
    ...
    buttons: [
        {
            text: '<i class="fas fa-file-excel"></i> Nuevo Grupo',
            titleAttr: 'Nuevo Grupo de servicio',
...
function iniciar() {
    ...
    get("/ModTablas/ManteServicioGruposCSV/?data=1|", mostrarLista);
    ...
    $('#tabla tbody').on('click', 'tr', function (e) {
        ...
        if (!(opc_sel === undefined)) {
            if (opc_sel == 2 || opc_sel == 3) {
                if (celdas[3] == "&lt;ACT&gt;") get("/ModTablas/ManteServicioGruposCSV/?data=" + opc_sel + "|" + celdas[1], editarLista);
                else {
                    Swal.fire({
                        icon: 'error',
                        title: "¡ Anulado !",
            ...
            if (opc_sel == 4) {
                Swal.fire({
                    ...
                    if (result.value) {
                        post("/ModTablas/ManteServicioGruposGrbCSV", mostrarGrabar, opc_sel + "|" + celdas[1]);
                    }
                });
            }
        }
    ...
    btnGuardarLista.onclick = function () {
        if (ControlesValidar() == 0) {
            ...
            post("/ModTablas/ManteServicioGruposGrbCSV", mostrarGrabar, "5|" + data);
        }
    }
...
function mostrarGrabar(rpta) {
    ...
    if (rpta.substr(0, 5) != "Error") {
        if (opc_sel == 1) {
            ...
            mensaje = "Grupo de servicio adicionado";
        }
        if (opc_sel == 3) {
            lista = rpta.split("|");
            celdas[2] = lista[2];
            ...
            mensaje = "Grupo de servicio actualizado";
        }
        if (opc_sel == 4) {
            lista = rpta.split("|");
            celdas[3] = lista[3];
            tabla.row(objfila).data(celdas).draw();
            mensaje = "Grupo de servicio anulado";
        }
        Swal.fire({
            icon: 'success',
            title: mensaje,
