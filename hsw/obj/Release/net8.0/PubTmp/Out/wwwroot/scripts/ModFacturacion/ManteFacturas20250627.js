...
var tabla = $('#tabla').DataTable({
    ...
    columnDefs: [
        ...
        {
            width: "6rem", targets: 11, className: "text-center",
            render: function (data, type, row) {
                let fecenv=''
...
function ControlesValidarNC() {
    eliminarMarcasNC();
    let validacion = 0;
    validacion += validaCampos('cboCliProNC', '-1');
    validacion += validaCampos('cboTabMotNC', '-1');
    return (validacion != 0) ? 1 : 0;
}
...
function eliminarMarcasB() {
    removeRedMark('cboCliProNC','cboTabMotNC');
}
