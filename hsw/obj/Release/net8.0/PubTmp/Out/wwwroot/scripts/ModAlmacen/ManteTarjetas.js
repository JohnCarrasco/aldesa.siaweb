...
var tabla = $('#tabla').DataTable({
    ...
    columnDefs: [
        {
            ...
            render: function (data, type, full, meta) {
                return (
                    ...
                    '<a data-bs-toggle="tooltip" class="btn btn-sm btn-icon btn-text-secondary waves-effect waves-light rounded-pill" data-bs-placement="top" title="Consultar tarjeta"><i class="ri-eye-line"          style="color:blue"  data-opc="con"></i></a>' +
                    '<a data-bs-toggle="tooltip" class="btn btn-sm btn-icon btn-text-secondary waves-effect waves-light rounded-pill" data-bs-placement="top" title="Editar tarjeta"   ><i class="ri-pencil-line"       style="color:green" data-opc="edt"></i></a>' +
                    '<a data-bs-toggle="tooltip" class="btn btn-sm btn-icon btn-text-secondary waves-effect waves-light rounded-pill" data-bs-placement="top" title="Eliminar tarjeta" ><i class="ri-delete-bin-7-line" style="color:red"   data-opc="anu"></i></a>' +
                    '<a data-bs-toggle="tooltip" class="btn btn-sm btn-icon btn-text-secondary waves-effect waves-light rounded-pill" data-bs-placement="top" title="Exportar Kardex"  ><i class="ri-store-line"        style="color:green" data-opc="kar"></i></a>' +
                    '</div>'
                );
            }
        },
        { targets: 1, width: "5rem", orderSequence: ['asc', 'desc'], className: "text-end" },
        {
            targets: [2, 11, 12], width: "6rem", className: "text-center",
        ...
        { targets: 3, width: "4rem", className: "text-center" },
        { targets: 4, width: "15rem", className: "text-wrap" },
        { targets: [5, 6], width: "5rem", className: "text-end", render: $.fn.dataTable.render.number(',', '.', 0) },
        { targets: [7, 8], width: "6rem", className: "text-end", render: $.fn.dataTable.render.number(',', '.', 2) },
        { targets: 9, width: "20rem", className: "text-wrap", orderable: false },
        { targets: 10, width: "11rem" },
        { targets: 13, width: "9rem" }
    ],
    ajax: (data, callback, settings) => {
        ...
        get("/ModAlmacen/ManteTarjetasCSV/?data=lst|" + cboServicio.value + '|' + scroll_param, (response) => {
    ...
    $('#tabla tbody').on('click', 'tr', function (e) {
        ...
        if (!(opc_sel === undefined)) {
            if (opc_sel == 'con' || opc_sel == 'edt') {
                ...
                if (opc_sel == 'kar') {
                    Swal.fire({
                        title: "¿ Seguro de exportar kardex al Excel ?",
                        text: "Exportar",
                        icon: "enabled",
                        ...
                        if (result.value) {
                            navegar("/ModReportes/RptKardex_XLS/?data=" + "|" + cboServicio.value + "|" + celdas[1]);
                        }
                    });
                } else {
