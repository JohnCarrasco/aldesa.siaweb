...
var tabla = $('#tabla').DataTable({
    ...
    responsive: { // Configuración explícita de Responsive
        details: { // Controla cómo se muestran los detalles de las columnas ocultas
            display: $.fn.dataTable.Responsive.display.modal({
                header: function (row) {
                    var data = row.data();
                    return 'Detalles de Tipo de Cambio para Fecha: ' + moment(data[1]).format('DD/MM/YYYY');
                }
            }),
    ...
    columnDefs: [
        ...
        { width: "10%", targets: 3, responsivePriority: 4, className: "dt-body-center text-end" },
        {
            targets: 4, responsivePriority: 5, className: "dt-body-center text-center",
            render: function (data, type, row) {
                let fechaMostrar = ''
                if (data == '0000-00-00 00:00:00') fechaMostrar = '';
        ...
        {
            targets: 5, responsivePriority: 6, className: "dt-body-center text-center",
            render: function (data, type, row) {
                let fechaMostrar = ''
                if (data == '0000-00-00 00:00:00') fechaMostrar = '';
                else fechaMostrar = moment(data).format('DD/MM/YYYY HH:mm:ss');
    ...
    buttons: [
        {
            ...
            action: function (e, dt, node, config) {
                opc_sel = 'adi';
                modalTitulo.innerHTML = 'Crear Tipo de cambio';
                dtpFecha.value = Hoy;
...
function iniciar() {
    ...
    get("/ModTablas/ManteTabCamCSV/?data=ini|", cargaInicial);

    cboPeriodos.onchange = function () {
        mostrarBloqueo('#lista-lst');
        get("/ModTablas/ManteTabCamCSV/?data=lst|" + cboPeriodos.value, mostrarLista);
    }
        ...
        if (!(opc_sel === undefined)) {
            if (celdas[10] == 0) {
                Swal.fire({
                    icon: 'error',
                    title: "¡ Anulada !",
                    text: "Tipo de cambio se encuentra anulado.",
                    customClass: {
                ...
                if ('con-edt'.includes(opc_sel)) {
                    if (opc_sel == 'con') modalTitulo.innerHTML = 'Consultar tipo de cambio';
                    else modalTitulo.innerHTML = 'Editar tipo de cambio';
                    get("/ModTablas/ManteTabCamCSV/?data=" + opc_sel + "|" + celdas[1], editarLista);
                }           }
        }
    });
    ...
    btnGuardarLista.onclick = function () {
        if (ControlesValidar() == 0) {
            ...
            post("/ModTablas/ManteTabCamGrbCSV", mostrarGrabar, "grb|" + data);
        }
    }
}
...
function ControlesValidar() {
    eliminarMarcas();
    let validacion = 0;
    validacion += validaCampos('dtpFecha', '');
    return (validacion != 0) ? 1 : 0;
}
function eliminarMarcas() {
    removeRedMark('dtpFecha');
}
function editarLista(rpta) {
    var reg = rpta.split('|');
    dtpFecha.value = reg[0];
    txtCompra.value = reg[1];
    txtVenta.value = reg[2];
    ...
    if (opc_sel == 'edt') {
        txtCompra.disabled = false;
        txtVenta.disabled = false;
        btnGuardarLista.disabled = false;
    }
    ListaAdiMod.show();
}
function obtenerDatosGrabar() {
    var data = dtpFecha.value + '|';
    data += txtCompra.value * 1 + '|';
    data += txtVenta.value * 1;
    return data;
}
function mostrarGrabar(rpta) {
    ...
    if (rpta.substr(0, 5) != "Error") {
        if (opc_sel == 'adi') {
            ...
            mensaje = "Tipo de cambio adicionado ";
        }
        if (opc_sel == 'edt') {
            lista = rpta.split("|");
            celdas[2] = lista[2];
        ...
        if (opc_sel == 'anu') {
            ...
            celdas[7] = lista[7];
            tabla.row(objfila).data(celdas).draw();
            mensaje = "Tipo de cambio anulado";
        }
        Swal.fire({
            icon: 'success',
            title: mensaje,
